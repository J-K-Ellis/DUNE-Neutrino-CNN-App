...
class Custom_Figure_Page(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        Custom_Plot_Names = [attr for attr in dir(self.controller.Custom_Plot) if callable(getattr(self.controller.Custom_Plot, attr)) and not attr.startswith("__")]
        ...
        if self.custom_fig_seleceted.get() in ['Track_dE_Analysis', 'Track_dE_Analysis_Thesis']:
            ...
            if not hasattr(self, "Download_dir"):
                self.Download_dir = tk.Button(
                    ...
                    command=lambda: self.controller.Custom_Plot.Track_dE_Analysis( self, {'Plot_Mode': 'Download_Dir', 'canvas': canvas, 'fig': self.custom_fig}) )
                self.Download_dir.pack(anchor='w', side=tk.LEFT)

            # Call the plotting function based on the selection
            getattr(self.controller.Custom_Plot, self.custom_fig_seleceted.get())(
                self,
